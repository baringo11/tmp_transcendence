		...
		<div class="score-board">
		  ...
		  <div v-if="gameEnded" class="popup">
			<div class="popup-content">
			  ...
			  <router-link to="/" class="button">OK</router-link>
			</div>
		  </div>
  ...
  <script lang="ts">
	...
	export default defineComponent({
	  ...
	  setup() {
		...
		const gameLevel = ref(game.gameCustomization.gameDifficult);
		const gameWinningScore = ref(game.gameCustomization.winningScore);
		...
		const quitGame = () => {
	 	...
	 	const setGameInfo = (gameInfo: GameInfo) => {
			game.scoreBoard.user1Name = gameInfo.player1;
			game.scoreBoard.user2Name = gameInfo.player2;
			game.gameCustomization.winningScore = gameInfo.winningScore;
			game.gameCustomization.gameDifficult = gameInfo.gameDifficult;
			gameLevel.value = gameInfo.gameDifficult;
			gameWinningScore.value = gameInfo.winningScore;
		}
  
		onMounted(() => {
			const id: string = router.currentRoute.value.params.matchId as string;
			matchId.value = id;

			user.socket?.on("spectate-game", (gameExists : Boolean) => {if (!gameExists) router.push("");});
			user.socket?.on("spectate-game-players", setGameInfo);
			user.socket?.emit("spectate-game", matchId.value);
			user.socket?.on('spectator-end-game', (winner) => {
		...
		onBeforeUnmount(() => {
			spectator.stopSpectating(matchId.value);
			user.socket?.off("spectate-game");
			user.socket?.off("spectate-game-players");
			user.socket?.off("spectator-end-game");
		});
  
		return { pongCanvas, gameLevel, gameWinningScore, gameStarted, gameEnded, gameWinner, scoreBoard, quitGame };
	  },
	});
